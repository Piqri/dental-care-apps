        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-gradient-to-r from-green-50 to-green-100 border border-green-200 rounded-xl p-6 shadow-sm">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-green-600">Perlu Tindakan</p>
                            <p class="text-3xl font-bold text-green-700 mt-2">{{ $stats['need_action'] ?? 0 }}</p>
                            <p class="text-xs text-green-500 mt-1">Konsultasi/Kontrol rutin</p>
                        </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($checkups as $checkup)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="h-10 w-10 bg-gradient-to-br from-pink-100 to-pink-200 rounded-full flex items-center justify-center shadow-sm">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-xs text-gray-500 mt-1 flex items-center gap-2">
                                                    <span>{{ $checkup->pasien->umur }} tahun</span>
                                                    <span>•</span>
                                                    <span>Trimester: {{ $checkup->pasien->trimester ?? '-' }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="max-w-[200px]">
                                            @php
                                                $keluhan_utama = [];
                                                if($checkup->gigi_sakit == 'Ya') $keluhan_utama[] = 'Gigi Sakit';
                                                if($checkup->gusi_sakit == 'Ya') $keluhan_utama[] = 'Gusi Sakit';
                                                if($checkup->gusi_bengkak == 'Ya') $keluhan_utama[] = 'Gusi Bengkak';
                                                if($checkup->gigi_berdarah == 'Ya') $keluhan_utama[] = 'Gigi Berdarah';
                                                if($checkup->dikomentari_bau_mulut == 'Ya') $keluhan_utama[] = 'Bau Mulut';
                                                if($checkup->gigi_goyang == 'Ya') $keluhan_utama[] = 'Gigi Goyang';
                                                if($checkup->sulit_mengunyah == 'Ya') $keluhan_utama[] = 'Sulit Mengunyah';
                                                if($checkup->makanan_terselip == 'Ya') $keluhan_utama[] = 'Makanan Terselip';
                                            @endphp
                                            
                                            @if(count($keluhan_utama) > 0)
                                                <div class="space-y-1">
                                                    @foreach(array_slice($keluhan_utama, 0, 3) as $keluhan)
                                                        <span class="inline-block px-2 py-1 bg-red-50 text-red-700 rounded text-xs font-medium">
                                                            {{ $keluhan }}
                                                        </span>
                                                    @endforeach
                                                    @if(count($keluhan_utama) > 3)
                                                        <span class="inline-block px-2 py-1 bg-gray-100 text-gray-600 rounded text-xs">
                                                            +{{ count($keluhan_utama) - 3 }} lainnya
                                                        </span>
                                                    @endif
                                                </div>
                                            @else
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="space-y-2 max-w-[200px]">
                                            ...
                                            @if(!$checkup->kondisi_karies && !$checkup->kondisi_sisa_akar && !$checkup->kondisi_karang_gigi && 
                                                !$checkup->kondisi_gusi_bengkak && !$checkup->kondisi_pendarahan && !$checkup->kondisi_gigi_goyang)
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="space-y-1 max-w-[150px]">
                                            ...
                                            @endif
                                            @if($checkup->saran_konsultasi != 'Ya' && $checkup->saran_kontrol_rutin != 'Ya')
                                                <span class="text-xs text-gray-400 italic">Tidak ada saran</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end items-center space-x-2">
                                            {{-- WhatsApp Button --}}
                                            <a href="{{ route('pregnant-dental-checkups.whatsapp', $checkup->id) }}" 
                                            ...
                                            {{-- Print Button --}}
                                            <a href="{{ route('pregnant-dental-checkups.print', $checkup->id) }}" 
                                            ...
                                            {{-- Edit Button --}}
                                            <a href="{{ route('pregnant-dental-checkups.edit', $checkup->id) }}" 
                                            ...
                                            {{-- Delete Button --}}
                                            <form action="{{ route('pregnant-dental-checkups.destroy', $checkup->id) }}" method="POST" 
