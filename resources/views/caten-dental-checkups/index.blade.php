                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900 line-clamp-2">
                                            @php
                                                $keluhan = [];
                                                if($checkup->gigi_berlubang) $keluhan[] = 'Gigi Berlubang';
                                                if($checkup->riwayat_sakit_gigi) $keluhan[] = 'Riwayat Sakit Gigi';
                                                if($checkup->gusi_bengkak) $keluhan[] = 'Gusi Bengkak';
                                                if($checkup->sisa_akar) $keluhan[] = 'Sisa Akar';
                                                if($checkup->gusi_berdarah) $keluhan[] = 'Gusi Berdarah';
                                                if($checkup->gigi_goyang) $keluhan[] = 'Gigi Goyang';
                                                if($checkup->sariawan) $keluhan[] = 'Sariawan';
                                            @endphp
                                            {{ implode(', ', $keluhan) }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-right text-sm">
                                        <div class="flex justify-end space-x-2">
                                            <a href="{{ route('caten-dental-checkups.show', $checkup->id) }}" class="p-2 rounded-full text-blue-600 hover:bg-blue-50" title="Lihat">
                                            ...
                                            <a href="{{ route('caten-dental-checkups.edit', $checkup->id) }}" class="p-2 rounded-full text-yellow-600 hover:bg-yellow-50" title="Edit">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>

                                            <form action="{{ route('caten-dental-checkups.destroy', $checkup->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data pemeriksaan ini?')">
                                                @csrf @method('DELETE')
